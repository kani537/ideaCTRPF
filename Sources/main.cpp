...
// HotKeyの部分
static MenuEntry* EntryWithHotkeys(MenuEntry* entry,
...
std::string SystemInfomation()
{
  ...
  std::string systeminfo =
    Utils::Format("%d/%02d/%02d(%s) %s:%02d:%02d:%02d", year, month,
                  day, kListTimeOfTheWeek[time_of_the_week].c_str(),
...
bool LoadGameTitle(const Screen& scr)
{
  ...
  if (scr.IsTop) {
    ...
    scr.DrawSysfont(tid + ":" << Color::LimeGreen << "対応ゲーム",
                    166, 22 * 10, Color::White);
  }
  return true;
}
...
bool checkPass(void)
{
  std::vector<u16> answer = {0x6A,  0x3EC, 0x2175, 0x29FB,
                             0x2CF, 0x5C,  0xBB};
  std::vector<u16> diff = {0x84,   0x25,  0x266, 0x2999,
                           0x2114, 0x37A, 0x7};
  ...
  for (int j = 0; j < answer_length; j++) {
    ...
    if (buff != (answer[j] + diff[j] + hash)) {
      ...
      key.CanAbort(false);
      key.Open(output);
      Process::WriteString((u32)&utf16,
                           output.substr(0, answer_length),
                           StringFormat::Utf16);
...
// This patch the NFC disabling the touchscreen when scanning an
// amiibo, which prevents ctrpf to be used
static void ToggleTouchscreenForceOn(void)
{
  ...
  found =
    (u32*)Utils::Search<u32>(0x14000000, (u32)textTotalSize, pattern);
  ...
  svcUnmapProcessMemoryEx(CUR_PROCESS_HANDLE, 0x14000000,
                          textTotalSize);
exit:
...
void InitMenu(PluginMenu& menu)
{
  ...
  *searchFolder += new MenuEntry("Search", nullptr, Search);
  ...
  menu += new MenuEntry("JPNotify", JPNotify,
                        "ジェーピーノティファイ\nstartで表示\n("
                        "Y押しながら押すんじゃないぞ！)");
  menu += new MenuEntry(
    "PlayMusic", nullptr, PlayMusic,
    "プレイミュージック\nMUSICフォルダにbcwavを入れてください");
  menu += new MenuEntry("ColorPicker", nullptr, ColorPicker,
                        "カラーピッカー");
  menu += EntryWithHotkeys(
    new MenuEntry("Tetris", nullptr, Tetris,
                  "テトリス\n一回Hotkeys "
                  "Modifierを見に行ってください\n操作できません\nMenu"
                  "Func(キーボードのボタン)から設定を変えれます"),
    {Hotkey(Key::DPadLeft, "左"), Hotkey(Key::DPadRight, "右"),
     Hotkey(Key::DPadDown, "下"),
     Hotkey(Key::DPadUp, "ハードドロップ"), Hotkey(Key::A, "右回転"),
     Hotkey(Key::B, "左回転"), Hotkey(Key::R, "ホールド"),
     Hotkey(Key::Y, "Pause")});
...
{
  PluginMenu* menu =
    new PluginMenu("IDEA CTRPF", 0, 7, 4, "made by kani537");
