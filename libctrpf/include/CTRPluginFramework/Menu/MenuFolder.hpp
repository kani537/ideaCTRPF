...
class MenuFolder {
 public:
   ...
   * Upon destruction all MenuEntry and MenuFolder objects contained
   * by this folder\n will be destroyed too (so all pointers will be
   * invalidated).
   */
  ~MenuFolder();

  // hide or show the folder from the menu.
   ...
   * counting subfolders's content) \return The count of items
   */
   ...
   * \param destroy If the elements must be destroyed (invalidate
   * pointers and release ressources)
   */
   ...
   * \n along with the action which triggered the callback \return
