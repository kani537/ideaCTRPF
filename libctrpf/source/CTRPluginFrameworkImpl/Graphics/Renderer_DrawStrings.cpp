...
void Renderer::DrawPixel(int posX, int posY, const Color& color)
{
  ...
  PrivColor::ToFramebuffer(screen->GetLeftFrameBuffer(posX, posY),
                           color);
}

// Draw Character without background
void Renderer::DrawCharacter(int c, int posX, int posY,
                             const Color& fg)
{
  ScreenImpl* screen = GetContext()->screen;
...
int Renderer::DrawString(const char* str, int posX, int& posY,
                         Color fg)
{
  Color bak = fg;
