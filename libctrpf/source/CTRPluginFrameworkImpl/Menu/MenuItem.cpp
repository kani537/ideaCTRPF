...
void MenuItem::Hide(void)
{
  ...
  // Else refresh the menu to be sure that we're not currently in a
  // hidden folder
  ...
  if (_container != nullptr) {
    MenuFolderImpl *container =
      reinterpret_cast<MenuFolderImpl *>(_container);
    std::vector<MenuItem *> &items = container->_items;

    items.erase(std::remove(items.begin(), items.end(), this),
                items.end());
  }
}
